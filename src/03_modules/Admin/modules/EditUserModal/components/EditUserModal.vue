...
import { patchAdminUser } from '../../../api/admin.api';
...
const { mutate, status: mutateStatus } = useMutation({
  mutation: (dto: Parameters<typeof patchAdminUser>[1]) => patchAdminUser(props.user.id, dto),
  onSuccess: () => {
    emit('saved');
    emit('close');
  },
  ...
  <dialog-root :open="true">
    <dialog-portal>
      ...
      <dialog-content class="edit-modal">
        <dialog-title class="edit-modal__title">{{ $t('admin.edit-user.title') }}</dialog-title>
        <p class="edit-modal__subtitle">{{ $t('admin.edit-user.subtitle', { name: user.name }) }}</p>

        <form class="edit-modal__form" novalidate @submit="onSubmit">
          <v-form-field :title="$t('admin.edit-user.name')" :error="errors.name">
            <v-input v-model="name" variant="soft" size="xl" name="name" />
          </v-form-field>
          <v-form-field :title="$t('admin.edit-user.email')" :error="errors.email">
            <v-input v-model="email" variant="soft" size="xl" name="email" type="email" />
          </v-form-field>
          <v-form-field :title="$t('admin.edit-user.role')" :error="errors.role">
            <select v-model="role" class="edit-modal__select">
              <option v-for="opt in roleOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
            </select>
          </v-form-field>
          <v-form-field :title="$t('admin.edit-user.group')">
            <v-input v-model="group" variant="soft" size="xl" name="group" />
          </v-form-field>
          <v-form-field :title="$t('admin.edit-user.status')" :error="errors.status">
            <select v-model="status" class="edit-modal__select">
              <option v-for="opt in statusOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
            </select>
          </v-form-field>

          <p v-if="serverError" class="edit-modal__error">{{ serverError }}</p>

          <div class="edit-modal__actions">
            <v-button :variant="ButtonVariants.OUTLINE" :size="ButtonSizes.MD" type="button" @click="emit('close')">
              {{ $t('admin.edit-user.cancel') }}
            </v-button>
            <v-button :type="ButtonTypes.SUBMIT" :size="ButtonSizes.MD" :is-loading="mutateStatus === 'pending'">
              {{ $t('admin.edit-user.save') }}
            </v-button>
          </div>
        </form>

        <button class="edit-modal__close" @click="emit('close')">×</button>
      </dialog-content>
    </dialog-portal>
  </dialog-root>
</template>
